...
<head>
    ...
    <link rel="stylesheet" href="thue_Traphong_Themmenu.css">
        ...
        <div class="container-content">
            ...
            <div class="box-form">
                <form action="" method="POST" class="form">
                    ...
                    <input type="submit" name="btn-tim-kiem" value="Tìm kiếm">

                    <?php
                        $loai_menu = (isset($_POST['loai-menu'])) ? $_POST['loai-menu'] : "Đồ uống";

                        $sql = "SELECT `MaMenu`, `TenMenu`
                        FROM `menu`
                        WHERE `LoaiMenu` = '{$loai_menu}'";
                        $rows = DataProvider::ExecuteQuery($sql);
                    ?>
                    <select name="ten-menu" id="ten-menu">
                    <?php
                        while($row = mysqli_fetch_array($rows)) {
                        ?>
                            <option value="<?php echo $row['MaMenu']; ?>"><?php echo $row['TenMenu']; ?></option>
                        <?php
                        }
                    ?>
                    </select>
                    <label for="" class="label-so-luong">Số lượng:</label>
                    <input type="number" name="so-luong" value=1 min=1>
                    <input type="submit" name="btn-them" value="Thêm">
                </form>
                <?php
                    ...
                    if(isset($_POST['btn-them'])) {
                        $ma_menu = (isset($_POST['ten-menu'])) ? $_POST['ten-menu'] : "";
                        $so_luong = (isset($_POST['so-luong'])) ? $_POST['so-luong'] : "";

                        $sql = "SELECT `DonGia`
                        FROM `menu`
                        WHERE `MaMenu` = {$ma_menu}";
                        $row = mysqli_fetch_array(DataProvider::ExecuteQuery($sql));
                        $don_gia = $row['DonGia'];

                        $tong_tien = $so_luong * $don_gia;

                        $sql = "INSERT INTO `sudungmenu`(`MaPhong`, `MaMenu`, `SoLuong`, `TongTien`) 
                        VALUES ({$ma_phong},{$ma_menu},{$so_luong},{$tong_tien})";
